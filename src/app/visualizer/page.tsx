          ...
          <div key={category.id} className="border p-4 rounded-lg shadow">
            <h2 className="text-xl font-semibold">
              <Link href={`/categories/${category.attributes.slug}`}>
                <span className="font-normal">Categoria: </span>{category.attributes.name}
              </Link>
            </h2>
            {category.attributes.description && (
              <p className="text-gray-600 mb-2">{category.attributes.description}</p>
            )}
            <h3 className="text-lg font-semibold mt-4 border-b border-black">Subcategorías:</h3>
